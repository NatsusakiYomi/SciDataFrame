...
SciDataFrame <- R6Class(
  ...
  public = list(
    ...
    filter=function(pattern){
      filtered_schema<-self$schema[grepl(pattern, self$schema$name), ]
      return(SciDataFrame$new(dataset_id=self$dataset_id,streaming=self$streaming,client=self$client,is_preprocess=self$is_preprocess,schema=filtered_schema))
    },
    ...
    open = function(Name) {
      ...
      tryCatch({
        ...
        # 创建SciDataFrame对象
        return(SciDataFrame$new(schema = schema_filtered,
                        data = data,
                        level = level,
                        client = self$client))
...
# 创建一个 SciDataFrame 对象
# ds <- SciDataFrame$new(client = local_client)
