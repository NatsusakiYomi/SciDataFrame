...
class PngParser(ParserInterface):

    def name(self):
        return "png_parser"

    def description(self):
        return "parse .png files to a dataframe of pixels"

    def parse(self, file_url) -> pa.Table:
        if file_url.startswith('file:'):
            temp_file_path = file_url[5:]
        else:
        ...
        t = t.replace_schema_metadata({
            'description': 'This dataframe is parsed by the default png_parser. Its data represents rgb values of every pixel.',
            'origin_format': 'png',
            'columns': "y, x, R, G, B",
